...
// Secondary app for user creation (doesn't sign out current admin)
const adminAuthApp = initializeApp(firebaseConfig, "AdminUserCreation");
const adminAuth = getAuth(adminAuthApp);
...
const DELEGATION_DISABLED_MESSAGE = 'Account delegation is disabled in mobile app mode.';
...
window.loadAccountDelegations = async () => {
    ...
    if (!listEl) return;
    listEl.innerHTML = '<div class="text-[10px] text-slate-500">Delegation is disabled.</div>';
};
...
const ADMIN_TABS = new Set([
    'overview', 'approvals', 'tasks', 'users', 'roles', 'projects',
    'workflow', 'reports', 'audit', 'chat', 'settings'
]);

const ADMIN_PORTAL_AUDIT_ROLES = new Set([
    'ADMIN', 'MANAGER', 'SENIOR_MANAGER', 'FINANCE_MANAGER', 'ACCOUNTS', 'TREASURY', 'AUDIT', 'HR'
]);

const syncSidebarSectionVisibility = () => {
    const sectionMap = {
    ...
    Object.entries(sectionMap).forEach(([sectionId, tabIds]) => {
        const sectionEl = document.querySelector(`[data-section="${sectionId}"]`);
        if (!sectionEl) return;
        const hasVisibleNav = tabIds.some((tabId) => {
            if (tabId === 'overview') {
                const overviewEl = document.querySelector('.sidebar-item[data-tab="overview"]');
                return !!overviewEl && !overviewEl.classList.contains('hidden');
            }
            const navEl = document.getElementById(`nav-${tabId}`);
            return navEl && !navEl.classList.contains('hidden');
        });
        if (hasVisibleNav) sectionEl.classList.remove('hidden');
...
window.initModulePreferencesDefaults = () => {
    ...
    const defaultHidden = ['crm', 'salary'];
    ...
    try {
        ...
        if (settingsSnap.exists()) {
            ...
            if (data.companyName) {
                ...
                if (sidebarName) sidebarName.textContent = data.companyName;
            }
            ...
            if (cmpSnap.exists()) {
                ...
                if (cmpData.name) {
                    ...
                    const sidebarName = document.getElementById('sidebar-company-name');
                    if (sidebarName) sidebarName.textContent = cmpData.name;
                }
                if (cmpData.logo) {
                    const logoImg = document.getElementById('login-logo-img');
                    const logoFallback = document.getElementById('login-logo-fallback');
...
if (tenantCompanyFromPath) {
    loadCompanyBranding(tenantCompanyFromPath);
}
        ...
        try {
            ...
            if (!snap.empty) {
                ...
                if (urlCompanyId && !userData.companyId) {
                    userData.companyId = urlCompanyId;
                }
                window.userData = userData;
                ...
                // If not an allowed management role, redirect to employee portal
                if (!allowedRoles.includes(userData.role)) {
                    // Inform user and redirect to employee portal
                    if (window.showToast) {
                ...
                const companyId = window.companyId || userData.companyId;
                ...
                // Calls disabled in mobile_exp build.
...
window.handleAccountActivation = async (e) => {
    ...
    if (!email || !pass || !confirm) return showToast("Please fill in all fields", "error");
    if (pass !== confirm) return showToast("Passwords do not match", "error");
    if ((window.currentPasswordStrength || 0) < 3) return showToast("Password does not meet all requirements", "warning");
...
window.checkAccess = async () => {
    ...
    if (dashLink) {
        dashLink.href = window.ExplyraTenant
            ? window.ExplyraTenant.toTenantAwareHref('admin.html', { companyId: window.companyId || userData?.companyId, forcePrefix: true })
            : 'admin.html';
        dashLink.querySelector('span').textContent = 'Admin Dashboard';
    }

    [
        'approvals', 'users', 'projects', 'reports', 'settings',
        'roles', 'audit', 'tasks', 'workflow', 'salary', 'chat'
    ].forEach((id) => {
    ...
    if (roleName !== 'ADMIN') {
        try {
            ...
            const roleDoc = roleSnap.docs.find(d => (d.data().name || '').toUpperCase() === roleName && (d.data().companyId === userData.companyId || d.data().companyId === 'GLOBAL'));
        ...
        userData.permissions = {
            ...
            viewTasks: true, viewProjects: true, viewRoles: true,
            viewWorkflow: true, viewAudit: true, viewSalary: true, viewChat: true
        };
    }

    const permissions = userData.permissions || {};
    const revealNavIfAllowed = (tabId, isAllowed) => {
        ...
        if (tabId !== 'overview' && !isTabVisibleInPrefs(tabId)) {
            nav.classList.add('hidden');
            return;
        }
    ...
    // Approvals
    revealNavIfAllowed('approvals', permissions.viewApprovals || ['ADMIN', 'MANAGER', 'SENIOR_MANAGER', 'FINANCE_MANAGER', 'ACCOUNTS', 'TREASURY'].includes(roleName));

    // User Mgmt
    revealNavIfAllowed('users', permissions.viewUsers || ['ADMIN', 'HR'].includes(roleName));

    // Projects
    revealNavIfAllowed('projects', permissions.viewProjects || ['ADMIN', 'HR'].includes(roleName));

    // Reports
    revealNavIfAllowed('reports', permissions.viewReports || ['ADMIN', 'TREASURY', 'SENIOR_MANAGER', 'FINANCE_MANAGER', 'ACCOUNTS'].includes(roleName));

    // Settings
    revealNavIfAllowed('settings', permissions.viewSettings === true || roleName === 'ADMIN');

    // Role Management (explicit permission only)
    revealNavIfAllowed('roles', permissions.viewRoles === true);

    // Audit Logs
    revealNavIfAllowed('audit', permissions.viewAudit === true || ADMIN_PORTAL_AUDIT_ROLES.has(roleName));

    // Task Manager
    revealNavIfAllowed('tasks', permissions.viewTasks || roleName === 'ADMIN' || roleName === 'MANAGER' || roleName === 'SENIOR_MANAGER');

    // Workflow: Admin only
    revealNavIfAllowed('workflow', roleName === 'ADMIN' || permissions.viewWorkflow);

    // Salary
    revealNavIfAllowed('salary', permissions.viewSalary || roleName === 'ADMIN' || roleName === 'FINANCE_MANAGER');

    // Chat
    revealNavIfAllowed('chat', permissions.viewChat === true || roleName === 'ADMIN');


    // --- GLOBAL VISIBILITY OVERRIDE ---
    // Double check specific toggles that might have been missed or are generic
    ['overview', 'approvals', 'tasks', 'salary', 'chat', 'reports', 'audit', 'users', 'roles', 'projects', 'workflow', 'settings'].forEach(id => {
        // Keep settings reachable for admin users even if sidebar prefs were changed.
        if (id === 'settings' && roleName === 'ADMIN') return;
    ...
    // Default Tab Logic
    if (roleName === 'HR' && window.hasAccessTo('users')) switchTab('users');
    else if (roleName === 'AUDIT' && window.hasAccessTo('audit')) switchTab('audit');
...
window.hasAccessTo = (tabId) => {
    ...
    if ((userData.role || '').toUpperCase() === 'ADMIN') return true;
    ...
    switch (tabId) {
        ...
        case 'projects': return p.viewProjects || p.viewUsers || r === 'HR';
        case 'roles': return p.viewRoles === true;
        ...
        case 'audit': return p.viewAudit || ADMIN_PORTAL_AUDIT_ROLES.has(r);
        case 'chat': return p.viewChat === true;
        case 'settings': return p.viewSettings === true;
...
window.switchTab = (tab, options = {}) => {
    let normalizedTab = normalizeAdminTab(tab);
    if (normalizedTab !== 'overview' && !window.hasAccessTo(normalizedTab)) {
    ...
    content.innerHTML = `
                    ...
                    <div class="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 fade-in">
                        <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4 flex items-center gap-2">
                            <i class="fa-solid fa-life-ring text-blue-600"></i> Quick Help
                        </h3>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mb-3">Need help with approvals, reports, or settings? Use support and docs links below.</p>
                        <div class="flex flex-wrap gap-2">
                            <a href="support.html" class="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition">Open Support</a>
                            <a href="help.html" class="px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-bold transition border border-slate-200 dark:border-slate-600">Help Center</a>
                        </div>
                    </div>
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    try {
        if (docId) {
            ...
            if (!snap.empty) {
                showToast('User with this email already exists in Firestore!', 'error');
                return;
            }
            ...
            try {
                ...
                if (authError.code === 'auth/email-already-in-use') {
                    showToast('Account already exists in Firebase Auth. Linking to this email.', 'info');
                    // In a real scenario, we might want to fetch the UID if possible, 
                    // but for simplicity, we'll proceed if the admin says so or just use the email as primary key if Firestore allows.
                    // But here we need the UID for proper linkage.
                } else {
            ...
            alert(successMsg); // Using alert for persistence, or I could use a custom modal
...
window.addEventListener('beforeinstallprompt', (e) => {
    if (isAdminStandaloneInstalled() || localStorage.getItem(ADMIN_INSTALL_ACCEPTED_KEY) === 'true') {
        markAdminInstallAccepted();
        return;
    }
...
window.toggleSidebar = () => {
    ...
    const isOpen = sidebar.classList.contains('sidebar-visible') || !sidebar.classList.contains('-translate-x-full');
    if (isOpen) {
        ...
        sidebar.classList.add('sidebar-visible', 'translate-x-0');
...
function ensureAdminChatScrollLatest() {
    ...
    let btn = document.getElementById('admin-chat-scroll-latest');
    if (!btn) {
        btn = document.createElement('button');
        btn.id = 'admin-chat-scroll-latest';
        btn.type = 'button';
        btn.className = 'hidden absolute bottom-20 right-4 z-30 h-9 px-3 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-700 dark:text-slate-200 shadow';
        btn.innerHTML = '<i class="fa-solid fa-angles-down mr-1"></i>Latest';
    ...
    container.addEventListener('scroll', () => {
        const awayFromBottom = (container.scrollHeight - container.scrollTop - container.clientHeight) > 160;
        btn.classList.toggle('hidden', !awayFromBottom);
    }, { passive: true });
}
...
function loadMessages() {
    ...
    try {
        ...
        chatUnsub = onSnapshot(q, (snapshot) => {
            if (!document.getElementById('chat-messages-emp') && !document.getElementById('chat-messages')) return;
            ...
            // Order is ASC (recent at bottom). Keep pinned only if user is already near bottom.
...
window.oldSendChatMessage = async (e) => {
    e.preventDefault();
    const input = document.getElementById('chat-input-emp') || document.getElementById('chat-input');
    const text = input.value.trim();
    if (!text) return;
...
window.openAccountCenter = async () => {
    ...
    try {
        ...
        const tabs = ['overview', 'approvals', 'tasks', 'users', 'projects', 'roles', 'workflow', 'salary', 'reports', 'audit', 'chat', 'settings'];
...
window.renderRoles = async () => {
    ...
    try {
        ...
        const defaultRoles = [
            { id: 'def-emp', name: 'EMPLOYEE', systemRole: true, description: 'Basic employee — can submit expense claims and view personal tasks', permissions: { viewApprovals: false, viewReports: false, viewChat: true } },
            { id: 'def-mgr', name: 'MANAGER', systemRole: true, description: 'Team manager — approves team expense claims and assigns tasks', permissions: { viewApprovals: true, viewReports: true, viewTasks: true, viewProjects: true, viewChat: true, viewAudit: true } },
            { id: 'def-smgr', name: 'SENIOR_MANAGER', systemRole: true, description: 'Senior manager — approves higher-value claims and manages users', permissions: { viewApprovals: true, viewReports: true, viewUsers: true, viewTasks: true, viewProjects: true, viewWorkflow: true, viewChat: true, viewAudit: true } },
            { id: 'def-fmgr', name: 'FINANCE_MANAGER', systemRole: true, description: 'Finance manager — reviews and approves financial claims', permissions: { viewApprovals: true, viewReports: true, viewChat: true, viewAudit: true } },
            { id: 'def-hr', name: 'HR', systemRole: true, description: 'Human resources — manages employee records and user accounts', permissions: { viewApprovals: false, viewReports: false, viewUsers: true, viewProjects: true, viewChat: true, viewAudit: true, viewRoles: true } },
            { id: 'def-accts', name: 'ACCOUNTS', systemRole: true, description: 'Accounts team — processes approved claims for payment', permissions: { viewApprovals: true, viewReports: true, viewChat: true, viewAudit: true } },
            { id: 'def-audit', name: 'AUDIT', systemRole: true, description: 'Auditor — reviews paid claims and generates audit reports', permissions: { viewApprovals: false, viewReports: true, viewChat: true, viewAudit: true } },
            { id: 'def-trsy', name: 'TREASURY', systemRole: true, description: 'Treasury — handles final payment disbursement', permissions: { viewApprovals: true, viewReports: true, viewChat: true, viewAudit: true } },
            { id: 'def-admin', name: 'ADMIN', systemRole: true, description: 'Full administrator — complete system access and control', permissions: { viewApprovals: true, viewReports: true, viewUsers: true, viewSettings: true, viewTasks: true, viewProjects: true, viewWorkflow: true, viewChat: true, viewAudit: true, viewRoles: true } }
        ];
                    ...
                    <form id="role-form" class="space-y-4">
                        ...
                        <div class="mt-4 border border-slate-200 dark:border-slate-700 rounded-xl p-4 bg-slate-50 dark:bg-slate-900/50">
                            ...
                            <div class="space-y-3" id="role-permissions-list">
                                ...
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-tasks" class="rounded text-green-600 focus:ring-green-500"> Can Access Task Manager
                                </label>
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-projects" class="rounded text-green-600 focus:ring-green-500"> Can Access Projects
                                </label>
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-workflow" class="rounded text-green-600 focus:ring-green-500"> Can Access Workflow
                                </label>
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-chat" class="rounded text-green-600 focus:ring-green-500"> Can Access Chat
                                </label>
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-audit" class="rounded text-green-600 focus:ring-green-500"> Can View Audit Logs
                                </label>
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                                    <input type="checkbox" id="perm-view-roles" class="rounded text-green-600 focus:ring-green-500"> Can Manage Roles
                                </label>
                            </div>
                        </div>
...
window.showRoleModal = (roleData = null) => {
    ...
    if (roleData) {
        ...
        document.getElementById('perm-view-tasks').checked = roleData.permissions?.viewTasks || false;
        document.getElementById('perm-view-projects').checked = roleData.permissions?.viewProjects || false;
        document.getElementById('perm-view-workflow').checked = roleData.permissions?.viewWorkflow || false;
        document.getElementById('perm-view-chat').checked = roleData.permissions?.viewChat || false;
        document.getElementById('perm-view-audit').checked = roleData.permissions?.viewAudit || false;
        document.getElementById('perm-view-roles').checked = roleData.permissions?.viewRoles || false;
    } else {
        ...
        document.getElementById('perm-view-tasks').checked = false;
        document.getElementById('perm-view-projects').checked = false;
        document.getElementById('perm-view-workflow').checked = false;
        document.getElementById('perm-view-chat').checked = false;
        document.getElementById('perm-view-audit').checked = false;
        document.getElementById('perm-view-roles').checked = false;
    }
