...
import { getAuth, signInWithEmailAndPassword, onAuthStateChanged, signOut, createUserWithEmailAndPassword, sendPasswordResetEmail, GoogleAuthProvider, signInWithPopup, deleteUser } from "https://www.gstatic.com/firebasejs/9.22.0/firebase-auth.js";
...
const ensureDashboardShellVisible = () => {
    ...
    const allSectionsHidden = [sectionClaims, sectionTasks, sectionFinancials]
        .filter(Boolean)
    ...
    [sectionClaims, sectionTasks, sectionFinancials].forEach((section) => {
        if (!section) return;
    ...
    if (user) {
        try {
            ...
            if (!snap.empty) {
                ...
                if (isDelegate) {
                    await signOut(auth);
                    const next = encodeURIComponent('emp.html');
        ...
        // User is signed out, use shared login flow
        const next = encodeURIComponent(window.location.pathname.split('/').pop() || 'emp.html');
