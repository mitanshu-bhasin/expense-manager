...
let mentionBindingsReady = false;
...
function ensureScrollToLatestButton() {
    ...
    btn.id = 'chat-scroll-latest-btn';
    btn.type = 'button';
    btn.className = 'hidden absolute bottom-3 right-3 z-20 h-8 px-3 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-700 dark:text-slate-200 shadow';
...
function runChatListener(collectionName, subCollectionId) {
    ...
    activeChatUnsub = onSnapshot(q, async (snapshot) => {
        ...
        const latestBtn = document.getElementById('chat-scroll-latest-btn');
        const awayFromBottom = (container.scrollHeight - container.scrollTop - container.clientHeight) > 180;
        if (!awayFromBottom) {
            container.scrollTop = container.scrollHeight;
