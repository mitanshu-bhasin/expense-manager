        ...
        await Promise.allSettled(
            PRECACHE_ASSETS.map(async (asset) => {
                try {
                    ...
                    // Optional file miss should not fail install.
                }
            })
        );
...
async function handleNavigation(event) {
    try {
        ...
        // Continue with network-first fallback.
    }

    const response = await networkFirst(event.request, CORE_CACHE, 4500);
...
self.addEventListener('push', (event) => {
    const data = event.data ? event.data.json() : { title: 'Explyra Update', body: 'New notification available.' };
    const options = {
        body: data.body,
        icon: './android-chrome-192x192.png',
        badge: './favicon-32x32.png',
        vibrate: [100, 50, 100],
        data: { url: data.url || './emp.html' }
    };
